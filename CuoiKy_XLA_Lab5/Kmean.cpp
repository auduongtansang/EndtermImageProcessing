...
int Kmean::Apply(const Mat & srcImage, Mat & dstImage)
{
	...
	//cout << tmp_rows << " " << tmp_cols << " " << tmp.channels() << endl;
	...
	*/
	...
	//for (int i = 0; i < this->_numClusters; i++) {
	//	std::cout << centroids[i].x << " , " << centroids[i].y << " , " << centroids[i].z << std::endl;
	//}

	/*
	* prepare base construction for clustering processing
	*/
	std::vector<trplit> cluster;
	std::vector<std::vector<trplit>> clusters;
